...
output.gwas<-read.csv('/Users/nunosep/Desktop/MiNI_PW/Xenstats/Data/COVID_19_GWAS_association_tests.csv',header=T)

#########################################################
######### PRODUCING MANHATTAN PLOT FOR COVID19 GWAS #####
#########################################################
...
qqunif(output.gwas[,'P'],main='Q-Q plot of Uniform distribution',ylim=c(0,10),xlim=c(0,8),las=1)
...
#########################################################
######### ADJUSTING P-VALUES (BH and BY)            #####
######### PRODUCING MANHATTAN PLOT FOR COVID19 GWAS #####
#########################################################

adj.pvalues.bh<-p.adjust(output.gwas[,'P'],method='BH')

plot(-log10(adj.pvalues.bh),cex=0.5,pch=19,col=as.character(colors),las=1,ylim=c(0,3),ylab=expression(-log[10]('adjusted p-value')),xlab='chromosome',axes=F,main='')
...
text(1,-log10(0.05)+0.15,'FDR',adj=0)